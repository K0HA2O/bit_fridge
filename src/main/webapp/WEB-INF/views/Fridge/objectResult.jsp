...
<body>
	...
	<main>
		<div class="container" style="width: 80%;">
			<h3 class="text-left mb-4">상품 인식 결과</h3>

			<form action="${pageContext.request.contextPath}/fridge/objectResult"
				method="post">
				<table>
					<thead>
						<tr>
					...
					<tbody>
						<c:if test="${not empty detectedObjects}">
							<c:forEach var="object" items="${detectedObjects}"
								...
								<tr>
									...
									<td><input type="text" class="form-control"
										name="name_${status.index}" value="${object}"></td>
									<td><input type="number" class="form-control"
										name="quantity_${status.index}" step="1" value="1"></td>
									<td><input type="text" class="form-control"
										name="unit_${status.index}" value="개"></td>
									<td><input type="date" class="form-control"
										name="expirationDate_${status.index}"></td>
								</tr>
							</c:forEach>
						</c:if>
						<c:if test="${empty detectedObjects}">
							<tr>
								<td colspan="5">상품 데이터가 없습니다.</td>
							</tr>
						</c:if>
					</tbody>
				</table>
