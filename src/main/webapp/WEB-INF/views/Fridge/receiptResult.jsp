...
<body>
	...
	<main>
		<div class="container" style="width: 80%;">
			<h3 class="text-left mb-4">영수증 인식 결과</h3>

			<form action="${pageContext.request.contextPath}/fridge/saveReceipt"
				method="post">
				<table>
					<thead>
						<tr>
					...
					<tbody>
						<c:if test="${not empty receiptItems}">
							<c:forEach var="item" items="${receiptItems}" varStatus="status">
								<tr>
									<td><input type="checkbox" name="selectedIndices"
										value="${status.index}"></td>
									<td><input type="text" class="form-control"
										name="name_${status.index}" value="${item.name}"></td>
									<td><input type="number" class="form-control"
										name="quantity_${status.index}" value="1" step="1"></td>
									<td><input type="text" class="form-control"
