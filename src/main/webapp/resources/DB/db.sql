...
-- 테이블 project.myrecipe 구조 내보내기
CREATE TABLE IF NOT EXISTS `myrecipe` (
  `id` int NOT NULL AUTO_INCREMENT,
  `user_id` int NOT NULL,
  `recipe_id` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
  `recipe_title` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
  `recipe_image` text CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
  `created_at` timestamp NOT NULL DEFAULT (now()),
  PRIMARY KEY (`id`) USING BTREE,
  KEY `FK_myrecipe_users` (`user_id`) USING BTREE,
  CONSTRAINT `FK_myrecipe_users` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- 테이블 project.recipecalendar 구조 내보내기
CREATE TABLE IF NOT EXISTS `recipecalendar` (
  ...
  `user_id` int NOT NULL,
  PRIMARY KEY (`id`) USING BTREE,
  KEY `FK_recipecalendar_users` (`user_id`) USING BTREE,
  CONSTRAINT `FK_recipecalendar_users` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS `userhealthdata` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT '데이터 고유 ID',
  `user_id` int NOT NULL COMMENT '사용자와 연결',
  `blood_sugar_level` float NOT NULL COMMENT '혈당 수치',
  `date` date NOT NULL DEFAULT (now()) COMMENT '데이터 입력 날짜',
  `meal_time` enum('아침','점심','저녁') CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '아침/점심/저녁',
  `meal_status` enum('전','후') CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '식사전, 식사후',
  `created_at` timestamp NOT NULL DEFAULT (now()) COMMENT '데이터 생성 시각',
  PRIMARY KEY (`id`) USING BTREE,
  KEY `FK_userhealthdata_users` (`user_id`) USING BTREE,
  CONSTRAINT `FK_userhealthdata_users` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
